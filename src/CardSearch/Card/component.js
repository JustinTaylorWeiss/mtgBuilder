    ...
    return <CardWrap>
        ...
        <CardQuantityWrap>
            {(card?.card_faces ?? false)
                    ? <>
                        <FlipCardButton data-tooltip-id={`flipCardTT${x},${y}`} onClick={() => setBackFace((prev) => !prev)}><FlipIcon height={70} src={rotate} alt="Flip Card"/></FlipCardButton>
                        <Tooltip id={`flipCardTT${x},${y}`} place="top" content="Flip Card"/>
                    </>
                    : <Spacer/>
            }
            <AddRemoveButton onClick={(e) => cardQuantityClick(e, -1)}><AddRemoveIcon draggable="false" height={60} src={minus} alt="-"/></AddRemoveButton>
            <CardQuantity $num={numOfCard}>{numOfCard}</CardQuantity>
            <AddRemoveButton onClick={(e) => cardQuantityClick(e, +1)}><AddRemoveIcon draggable="false" height={60} src={plus} alt="+"/></AddRemoveButton>
            <AddToDeckButton data-tooltip-id="addToDeckTT" onClick={() => addCardToDeckList(cardName, numOfCard)}><AddToDeckIcon height={60} src={addToDeck} alt="Add to deck"/></AddToDeckButton>
            <Tooltip id="addToDeckTT" place="top" content="Add to Deck"/>
        </CardQuantityWrap>
    </CardWrap>
};